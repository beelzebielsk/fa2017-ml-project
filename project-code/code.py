...
class AutoEncoder:
    def __init__(self, inputSize, hiddenLayerSizes):
        encoderLayers = tuple(inputSize) + hiddenLayerSizes
        decoderLayers = hiddenLayerSizes[::-1] + tuple(inputSize)
        encoderVariables = []
        encoderBiases = []
        decoderVariables = []
        decoderBiases = []
...
with tf.Session() as sess:
    ...
    for i in range(1, numSteps + 1):
        ...
        __, l = sess.run([optimizer, loss], feed_dict={X:batchX})
        if i % displayStep == 0 or i == 1:
            print("Step {}: Minibatch Loss: {}".format(i, l))

    original = data.test.nextBatch(20)
    modified = sess.run(decoderOp, feed_dict={X:original})
